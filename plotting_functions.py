...
def _prepare_data_nagents(path: str):
    if path == "decentralised":
        data = plot_over({"eta_lsa": "0.3",}, by="nagents", path="results/nagents",)
    else:
...
def plot_nagents(ax, path):
    ...
    sns.lineplot(
        ...
        style="Agent",
        legend=False,
        err_kws=dict(capsize=3, capthick=2,),
    )
...
def _plot_swapacc_and_agreement(
    path_agreement: str, path_swapacc: str, plotname: str, parameter_configs: list = None,
):
    prepare_plot()
    fig = get_fig((1, 2), size="beamer")
    gs = fig.add_gridspec(1, 2)
