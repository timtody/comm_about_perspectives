...
def compute_curve(
    X: np.ndarray, y: np.ndarray, ae: CifarAutoEncoder, sizes: list, rank: int
) -> pd.DataFrame:
    data = []
    for size in sizes:
        indices = np.random.randint(len(y), size=size)
...
def gather_results(
    X: np.ndarray, y: np.ndarray, sizes: list, seeds: int, weights_path: str, use_gpu: bool
) -> pd.DataFrame:
    results = [
...
def main(args: argparse.Namespace):
    ...
    results = gather_results(X, y, args.sizes, args.seeds, args.weights_path, args.use_gpu)
